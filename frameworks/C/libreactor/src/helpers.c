...
void write_response(stream *stream, segment preamble, segment body)
{
  ...
  // stream_allocate returns a segment which we convert to a char * pointer
   char *output_buffer = (char *) (stream_allocate(stream, response_size)).base;

  // memcpy the response directly to the output stream buffer
  memcpy(output_buffer, preamble.base, preamble.size);
  memcpy(output_buffer + preamble.size, date_header.base, date_header.size);
  memcpy(output_buffer + preamble.size + date_header.size, content_length_header.base, content_length_header.size);
  memcpy(output_buffer + preamble.size + date_header.size + content_length_header.size, body.base, body.size);
}

void plaintext(server_context *context, char *response)
{
  static segment text_preamble = { .base = TEXT_PREAMBLE, .size = sizeof(TEXT_PREAMBLE) - 1 };
...
{
  static segment json_preamble = { .base = JSON_PREAMBLE, .size = sizeof(JSON_PREAMBLE) - 1 };
